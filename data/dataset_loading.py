...
def get_case_identifiers_from_raw_folder(folder):
    case_identifiers = np.unique([
        i[:-12] for i in os.listdir(folder)
...
def load_dataset(
    folder,
    num_cases_properties_loading_threshold=1000,
    ...
    image = [m['image'] for m in meta_data]
    ...
    # "/media/lichangyong/Flash/服务器文件备份/lxp_temp/3D-TransUNet/nn_transunet/t5/t5-base/")
    ...
    for c in case_identifiers:
        ...
        indices = [index for index, value in enumerate(image) if value == c]
        i = indices[0]
        sentence = meta_data[i]['sentence']
        dataset[c]['splitBy'] = meta_data[i]['splitBy']
        ...
        dataset[c]['attention_mask'] = attn_mask
        dataset[c]['sentence'] = input_idss

    if len(case_identifiers) <= num_cases_properties_loading_threshold:
        print('loading all case properties')
        for i in dataset.keys():
            dataset[i]['properties'] = load_pickle(
                dataset[i]['properties_file'])
...
def crop_2D_image_force_fg(img, crop_size, valid_voxels):
    ...
    if len(valid_voxels) == 0:
        ...
        selected_center_voxel = valid_voxels[
            np.random.choice(valid_voxels.shape[1]), :]

    selected_center_voxel = np.array(selected_center_voxel)
    for i in range(2):
        selected_center_voxel[i] = max(crop_size[i] // 2,
                                       selected_center_voxel[i])
        selected_center_voxel[i] = min(
            img.shape[i + 1] - crop_size[i] // 2 - crop_size[i] % 2,
            selected_center_voxel[i])

    result = img[:, (selected_center_voxel[0] -
                     crop_size[0] // 2):(selected_center_voxel[0] +
                                         crop_size[0] // 2 + crop_size[0] % 2),
                 (selected_center_voxel[1] -
                  crop_size[1] // 2):(selected_center_voxel[1] +
                                      crop_size[1] // 2 + crop_size[1] % 2)]
    return result
...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            #! 2 ifs
            if 'sentence' in self._data[i].keys():
                sentence = self._data[i]['sentence']
            else:
                sentence = [0] * max_tokens
            case_sentences.append(sentence)
            if 'attention_mask' in self._data[i].keys():
                attn = self._data[i]['attention_mask']
            else:
                attn = [1] * max_tokens
            case_attentiion_mask.append(attn)
            ...
            if self.has_prev_stage:
                if isfile(self._data[i]['seg_from_prev_stage_file'][:-4] +
                          ".npy"):
            ...
            for d in range(3):
                ...
                # always
                if need_to_pad[d] + case_all_data.shape[
                        d + 1] < self.patch_size[d]:
        ...
        #! 2 rows
        import torch
        case_sentences = torch.tensor(case_sentences)
        case_attentiion_mask = torch.tensor(case_attentiion_mask)

        return {
            'data': data,
            'seg': seg,
            'sentence': case_sentences,
            'attention_mask': case_attentiion_mask,

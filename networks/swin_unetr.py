...
class SwinUNETR(nn.Module):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            self.swinViT.patch_embed.proj.weight.copy_(
                weights["swin.embeddings.patch_embeddings.projection.weight"])
            self.swinViT.patch_embed.proj.bias.copy_(
                weights["swin.embeddings.patch_embeddings.projection.bias"])
            for bname, block in self.swinViT.layers1[0].blocks.named_children(
            ):
                block.load_from(weights, n_block=bname, layer="layers1")
            self.swinViT.layers1[0].downsample.reduction.weight.copy_(
                weights["swin.encoder.layers.0.downsample.reduction.weight"])
            self.swinViT.layers1[0].downsample.norm.weight.copy_(
                weights["swin.encoder.layers.0.downsample.norm.weight"])
            self.swinViT.layers1[0].downsample.norm.bias.copy_(
                weights["swin.encoder.layers.0.downsample.norm.bias"])
            for bname, block in self.swinViT.layers2[0].blocks.named_children(
            ):
                block.load_from(weights, n_block=bname, layer="layers2")
            self.swinViT.layers2[0].downsample.reduction.weight.copy_(
                weights["swin.encoder.layers.1.downsample.reduction.weight"])
            self.swinViT.layers2[0].downsample.norm.weight.copy_(
                weights["swin.encoder.layers.1.downsample.norm.weight"])
            self.swinViT.layers2[0].downsample.norm.bias.copy_(
                weights["swin.encoder.layers.1.downsample.norm.bias"])
            for bname, block in self.swinViT.layers3[0].blocks.named_children(
            ):
                block.load_from(weights, n_block=bname, layer="layers3")
            self.swinViT.layers3[0].downsample.reduction.weight.copy_(
                weights["swin.encoder.layers.2.downsample.reduction.weight"])
            self.swinViT.layers3[0].downsample.norm.weight.copy_(
                weights["swin.encoder.layers.2.downsample.norm.weight"])
            self.swinViT.layers3[0].downsample.norm.bias.copy_(
                weights["swin.encoder.layers.2.downsample.norm.bias"])
    ...
    def forward(self, x_in,l=None,l_mask=None):
        if not torch.jit.is_scripting():
            self._check_input_size(x_in.shape[2:])
        hidden_states_out = self.swinViT(x_in, self.normalize,l,l_mask)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.use_checkpoint:
            x = checkpoint.checkpoint(self.forward_part1,
                                      x,
                                      mask_matrix,
                                      use_reentrant=False)
        else:
        ...
        if self.use_checkpoint:
            x = x + checkpoint.checkpoint(
                self.forward_part2, x, use_reentrant=False)
        else:
    ...
    def __init__(
        ...
        if is_fusion:
            ...
            self.res_gate = nn.Sequential(
                nn.Linear(dim * 2, dim * 2, bias=False), nn.ReLU(),
                nn.Linear(dim * 2, dim * 2, bias=False), nn.Tanh())
        else:
            self.fusion=None
            self.res_gate=None
    ...
    def forward(self, x,l=None,l_mask=None):
        ...
        if len(x_shape) == 5:
            ...
            if self.fusion is not None:
                # todo: here fusion,the input dim is dim*2
                B, D, H, W, C= x.size()
                x = x.view(B, -1, C)
                x_res = self.fusion(x, l, l_mask)  # input: (B,H*W*D,dim)
                # language gate
                x = x + (self.res_gate(x_res) * x_res)
...
class PWAM(nn.Module):
    ...
    def __init__(self,
        ...
        self.vis_project = nn.Sequential(
            nn.Conv1d(dim, dim, 1,
                      1),  # the init function sets bias to 0 if bias is True
            nn.GELU(),
            nn.Dropout(dropout))
        ...
        self.project_mm = nn.Sequential(
            nn.Conv1d(value_channels, value_channels, 1, 1), nn.GELU(),
            nn.Dropout(dropout))
...
class SpatialImageLanguageAttention(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        query = query.reshape(B, HW, self.num_heads,
                              self.key_channels // self.num_heads).permute(
                                  0, 2, 1, 3)
        ...
        out = torch.matmul(sim_map, value.permute(
            0, 1, 3, 2))  # (B, num_heads, H*W, self.value_channels//num_heads)
        out = out.permute(0, 2, 1, 3).contiguous().reshape(
            B, HW, self.value_channels)  # (B, H*W, value_channels)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, normalize=True,l=None,l_mask=None):
        x0 = self.patch_embed(x)
        x0 = self.pos_drop(x0)
        x0_out = self.proj_out(x0, normalize)
        if self.use_v2:
            x0 = self.layers1c[0](x0.contiguous())
        x1 = self.layers1[0](x0.contiguous(),l,l_mask)
        x1_out = self.proj_out(x1, normalize)
        if self.use_v2:
            x1 = self.layers2c[0](x1.contiguous())
        x2 = self.layers2[0](x1.contiguous(),l,l_mask)
        x2_out = self.proj_out(x2, normalize)
        if self.use_v2:
            x2 = self.layers3c[0](x2.contiguous())
        x3 = self.layers3[0](x2.contiguous(),l,l_mask)
        x3_out = self.proj_out(x3, normalize)
        if self.use_v2:
            x3 = self.layers4c[0](x3.contiguous())
        x4 = self.layers4[0](x3.contiguous(),l,l_mask)

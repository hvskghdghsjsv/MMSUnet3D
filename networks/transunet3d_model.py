...
class StackedConvLayers(nn.Module):
    def __init__(self, input_feature_channels, output_feature_channels, num_convs,
                 ...
                 norm_op=nn.BatchNorm2d, norm_op_kwargs=None,
                 dropout_op=nn.Dropout2d, dropout_op_kwargs=None,
                 nonlin=nn.LeakyReLU, nonlin_kwargs=None, first_stride=None, basic_block=ConvDropoutNormNonlin,fusion_num_heads=1,fusion_drop=0.0,is_fusion=False):
        '''
        ...
        if is_fusion:
            self.fusion = PWAM(output_feature_channels,
            ...
            self.res_gate = nn.Sequential(
                nn.Linear(output_feature_channels,
                        output_feature_channels,
                        bias=False), nn.ReLU(),
                nn.Linear(output_feature_channels,
                        output_feature_channels,
    ...
    def forward(self, x, l=None, l_mask=None):
        ...
        x = x.view((B, -1, C))  #(B,HxWxDi,C)

        # cross-modal fusion
        x_res = self.fusion(x, l, l_mask)
        # language gate
...
class Generic_TransUNet_max_ppbp(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for d in range(num_pool):
            ...
            self.conv_blocks_context.append(StackedConvLayers(input_features, output_features, num_conv_per_stage,
                                                              ...
                                                              first_stride, basic_block=basic_block,is_fusion=is_fusion))
        ...
        if self.is_max_bottleneck_transformer:
            ...
            if is_vit_pretrain:
                ...
                # self.transformer.load_from(
                #     weights=np.load("/home/lichangyong/Documents/lxp/3D-TransUNet/nn_transunet/networks/vit_checkpoint/imagenet21k/R50+ViT-B_16.npz"))
    ...
    def forward(self, x, l,l_mask):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            x = self.conv_blocks_context[d](x,l,l_mask)
            skips.append(x)
            if not self.convolutional_pooling:
                x = self.td[d](x)
...
class Generic_TransUNet_max_ppbp_x_MHA(SegmentationNetwork):
    DEFAULT_BATCH_SIZE_3D = 2
    DEFAULT_PATCH_SIZE_3D = (64, 192, 160)
    SPACING_FACTOR_BETWEEN_STAGES = 2
    BASE_NUM_FEATURES_3D = 30
    ...
    def forward(self, x, l,l_mask):
        ...
        if self.is_max:
            ...
            predictions = self.predictor(transformer_decoder_in_feature, mask_features, l, mask=None)

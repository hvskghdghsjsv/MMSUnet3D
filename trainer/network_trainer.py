...
def maybe_to_torch(d):
    if isinstance(d, list):
        d = [
            maybe_to_torch(i) if not isinstance(i, torch.Tensor) else i
            for i in d
        ]
...
class NetworkTrainer(object):
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        save_this = {
            'epoch':
            self.epoch + 1,  # actually saved epoch + 1...
            'state_dict':
            state_dict,
    ...
    def run_iteration(self,
        ...
        #! 4 rows
        sentences = data_dict['sentence']
        attentions = data_dict['attention_mask']
        sentences = maybe_to_torch(sentences)
        attentions = maybe_to_torch(attentions)
        ...
        if torch.cuda.is_available():
            ...
            attentions = to_cuda(attentions)

        self.optimizer.zero_grad()

        #! 3 rows
        last_hidden_states = self.text_model(sentences,
                                             attention_mask=attentions)[0]
        # last_hidden_states = self.text_model.encoder(sentences,attention_mask=attentions)[0]
        ...
        if self.fp16:
            with autocast():
                output = self.network(data, l=embedding, l_mask=attentions)

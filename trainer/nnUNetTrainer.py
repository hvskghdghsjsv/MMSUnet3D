...
from nn_transunet.networks.nnunet_model import Generic_UNet_with_PWAM, Generic_UNet

from ..networks.swin_unetr import SwinUNETR
from ..networks.swin_unetr import SwinUNETR_with_PWAM
...
class nnUNetTrainer(NetworkTrainer):
        ...
        if fold is not None:
            if isinstance(fold, str):
                assert fold.startswith(
                    "all"), "if self.fold is a string then it must be \'all\'"
    ...
    def initialize_network(self):
        ...
        if self.model == 'Generic_UNet':

            self.network = Generic_UNet(
                self.num_input_channels, self.base_num_features,
                self.num_classes, net_numpool, self.conv_per_stage, 2, conv_op,
        ...
        elif self.model == 'Generic_UNet_with_PWAM':
            self.network = Generic_UNet_with_PWAM(
                self.num_input_channels, self.base_num_features,
                self.num_classes, net_numpool, self.conv_per_stage, 2, conv_op,
                norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs,
                net_nonlin, net_nonlin_kwargs, False, False, lambda x: x,
                InitWeights_He(1e-2), self.net_num_pool_op_kernel_sizes,
                self.net_conv_kernel_sizes, False, True, True)
        elif self.model == 'Generic_UNet_with_PWAM_bot':
            from nn_transunet.networks.nnunet_model import Generic_UNet_with_PWAM_bot
            self.network = Generic_UNet_with_PWAM_bot(
                self.num_input_channels, self.base_num_features,
                self.num_classes, net_numpool, self.conv_per_stage, 2, conv_op,
                norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs,
                net_nonlin, net_nonlin_kwargs, False, False, lambda x: x,
                InitWeights_He(1e-2), self.net_num_pool_op_kernel_sizes,
                self.net_conv_kernel_sizes, False, True, True)
        elif self.model == 'Generic_SwinUNETR':
        ...
        elif self.model == 'Generic_MDUNET':
            from nn_transunet.networks.model import MDUNet as DynUNet
            self.network = DynUNet(spatial_dims=3,
                                   in_channels=1,
                                   out_channels=2,
            ...
            self.network = SwinUNETR_with_PWAM(
                img_size=(128, 128, 128),
                in_channels=1,
                out_channels=2,
                feature_size=48,
        ...
        # self.text_model = T5Model.from_pretrained("/media/lichangyong/Flash/服务器文件备份/lxp_temp/3D-TransUNet/nn_transunet/t5/t5-base")
    ...
    def get_basic_generators(self):
        self.load_dataset(
        )  # todo：get self.dataset(dict)  dataset contains 'data_file' 'properties_file' 'sentence' 'splitBy'
        self.do_split()
    ...
    def validate(self,
        ...
        for k in self.dataset_val.keys():
            ...
            if overwrite or (not isfile(join(output_folder, fname + ".nii.gz"))) or \
                ...
                data_dict = self.dataset[k]
                data = np.load(data_dict['data_file'])['data']
                ...
                #! 3 rows
                last_hidden_states = self.text_model(
